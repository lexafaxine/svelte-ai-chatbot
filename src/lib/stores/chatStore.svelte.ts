...
function createChatStore() {
	...
	function appendMessage(
		...
		if (role === 'user' && parentId === null && conv && !conv.titleAutoGenerated) {
	...
	async function generateTitle(conversationId: string, userText: string, assistantText: string) {
		const conv = getConversation(conversationId);
		if (!conv || conv.titleAutoGenerated) return;
		...
		try {
			...
			// Re-check: conversation may have been deleted or already titled while we awaited.
			const latest = getConversation(conversationId);
			if (!clean || !latest || latest.titleAutoGenerated) return;
			touchConversation(conversationId, { title: clean, titleAutoGenerated: true });
		} catch {
